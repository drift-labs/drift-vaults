...
import { Account, Mint, unpackAccount } from '@solana/spl-token';
import { isVersionedTransaction } from './testHelpers';
import { getAccount, getMint } from 'spl-token-bankrun';

export const TEST_ADMIN_KEYPAIR = [54,38,65,230,101,94,76,41,37,158,168,53,98,164,87,79,215,170,58,167,3,190,216,216,145,137,204,17,67,237,24,19,45,172,46,221,212,152,13,196,222,175,43,21,168,63,5,227,39,148,175,49,153,250,52,79,21,72,72,13,155,220,155,177];
...
export class BankrunConnection {
	...
	async sendRawTransaction(
		...
		try {
			...
			const tx = VersionedTransaction.deserialize(rawTransaction as Uint8Array);
			const signature = await this.sendTransaction(tx);
			return signature;
		}
	}
	...
	async getBalance(publicKey: PublicKey, commitment?: Commitment): Promise<bigint> {
		return this._banksClient.getBalance(publicKey, commitment);
	}
	...
	async getTokenAccountBalance(
		...
		return await getAccount(this._banksClient, tokenAddress, commitment);
	}
	...
	async getMint(
		...
		return await getMint(this._banksClient, mintAddress, commitment);
	}
}
