...
impl<'info> InitializeCompetitorCPI for Context<'_, '_, '_, 'info, InitializeCompetitor<'info>> {
  fn drift_competition_initialize_competitor(&self) -> Result<()> {
    ...
    let drift_competitions_program = self.accounts.drift_competitions_program.to_account_info().clone();
    let cpi_context = CpiContext::new_with_signer(drift_competitions_program, cpi_accounts, seeds).with_remaining_accounts(self.remaining_accounts.into());
